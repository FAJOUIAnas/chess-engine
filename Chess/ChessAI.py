...
def findBestMove(gs, validMoves, returnQueue):
    ...
    random.shuffle(validMoves)
    counter = 0
    # findMinMaxMoveRec(gs, validMoves, DEPTH, gs.whiteToMove)
...
def findMinMaxMoveRec(gs, validMoves, depth, whiteToMove):
    global nextMove
    if depth == 0:
    ...
    if whiteToMove:
        ...
        for move in validMoves:
            ...
            score = findMinMaxMoveRec(gs, nextMoves, depth - 1, False)
            if score > maxScore:
                maxScore = score
                if depth == DEPTH:
                    nextMove = move
        ...
        for move in validMoves:
            gs.makeMove(move)
            nextMoves = gs.getValidMoves()
            score = findMinMaxMoveRec(gs, nextMoves, depth - 1, True)
            if score < minScore:
                minScore = score
                if depth == DEPTH:
                    nextMove = move
...
def findNegaMaxMoveAlphaBeta(gs, validMoves, depth, alpha, beta, turnMultiplier):
    global nextMove, counter
    counter += 1
    if depth == 0:
        return turnMultiplier * scoreBoard(gs)
    ...
    for move in validMoves:
        gs.makeMove(move)
        nextMoves = gs.getValidMoves()
        score = -findNegaMaxMoveAlphaBeta(gs, nextMoves, depth - 1, -beta, -alpha, -turnMultiplier)
        if score > maxScore:
            maxScore = score
            if depth == DEPTH:
                nextMove = move
